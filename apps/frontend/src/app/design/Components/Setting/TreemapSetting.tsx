...
const DataMapper = ({ data, onMap, onBack }) => {
    ...
    const handleGenerate = () => {
        ...
        try {
            const mappedData = data.data
                .map(row => ({
                    id: String(row[nameField] || ''), // The ID is now derived from the name
                    parent: '', // All items will be at the root level
    ...
    return (
        <div className="space-y-4 p-4 border bg-white rounded-lg">
            ...
            <div>
                <label className="block text-sm font-medium text-gray-600 mb-1">Name Column (String)</label>
                <select 
                    value={nameField} 
            ...
            <div>
                <label className="block text-sm font-medium text-gray-600 mb-1">Value Column (Numeric)</label>
                <select 
                    value={valueField} 
