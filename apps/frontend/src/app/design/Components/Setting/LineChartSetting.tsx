...
const LineChartSetting = ({ initialData, onUpdate, onClose }) => {
    ...
    const handleRegenerateChart = () => {
        if (!selectedDataSet || !xAxisColumn || yAxisColumns.length === 0) {
            alert('Please select a data source and configure both X and Y axes.');
            return;
        }
                                ...
                                <div className="border border-gray-200 rounded-lg p-4">
                                    ...
                                    <div className="max-h-32 overflow-y-auto space-y-2">
                                        {availableColumns.filter(col => col !== xAxisColumn).map(column => {
                                            ...
                                            return (
                                                <label key={column} className="flex items-center px-1 py-0.5 rounded cursor-pointer hover:bg-gray-50">
                                                    <input
                                                        type="checkbox"
