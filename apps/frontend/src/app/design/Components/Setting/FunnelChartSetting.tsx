            ...
            {uniqueCategories.length > 0 && (
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <label className="text-sm font-medium text-gray-700">Filter Stages</label>
                    ...
                    <div className="max-h-32 overflow-y-auto p-2 border bg-white rounded-md space-y-1">
                        {uniqueCategories.map(category => (
                            <div key={category} className="flex items-center">
                                <input id={`cb-funnel-edit-${category}`} type="checkbox" checked={selectedCategories.includes(category)} onChange={() => handleCategoryToggle(category)} className="h-4 w-4 rounded border-gray-300" />
                                <label htmlFor={`cb-funnel-edit-${category}`} className="ml-2 block text-sm text-gray-900">{String(category)}</label>
                            </div>
                        ))}
                    </div>
                </div>
