        ...
        <div className="relative w-[320px] h-[94vh] bg-white border-r-2 border-gray-300 overflow-hidden">
            ...
            <div className="absolute top-0 left-0 w-full h-full transition-transform duration-300 ease-in-out">
                ...
                {selectedAi ? (
                    ...
                    <aside className="w-full h-full border-r border-gray-200 overflow-y-auto">
                        {/* Main Element List */}
                        <div className='px-3 pt-2 w-full grid grid-cols-1 gap-2 justify-center item-center rounded'>
                            <button
                                onClick={() => setSelectedAi(true)}
                                className={`p-2 text-black bg-gray-100 rounded text-xs cursor-pointer hover:bg-white hover:border hover:border-2 hover:border-gray hover:bg-gradient-to-r from-blue-800 via-indigo-700 to-purple-600 hover:text-white`}>
                                Create With Ai
                            </button>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 p-2 ">
                            {ElementsList.map((element, index) => (
                                <div
                                    ...
                                    className="flex flex-col justify-center items-center py-2 rounded-sm border cursor-pointer group bg-white hover:bg-indigo-50 border-gray-200"
                                >
                                    <div className="text-indigo-600 text-2xl">{element.icon}</div>
                                    <div className="text-[13px] font-medium text-center text-gray-700">{element.name}</div>
                                </div>
                            ))}
                        </div>
                    </aside>
            ...
            <Dialog.Root open={popupOpen} onOpenChange={handlePopupClose}>
                <Dialog.Portal>
                    ...
                    <Dialog.Content className="fixed top-1/2 left-1/2 overflow-y-auto z-50 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-2xl w-[40vw]">
                        <div className="flex justify-between items-center p-4 border-b ">
                            <Dialog.Title className="text-xl text-black font-semibold">{selectedOption?.name}</Dialog.Title>
                            <Dialog.Close asChild>
                                <button className="p-1 rounded-full text-black hover:bg-gray-200" aria-label="Close"><X size={20} /></button>
                            </Dialog.Close>
                        </div>
