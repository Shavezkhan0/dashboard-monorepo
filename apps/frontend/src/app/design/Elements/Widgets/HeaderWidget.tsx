...
export default function HeaderWidget({ title, logoUrl, metrics }) {
    ...
    return (
        <div className="w-full h-full bg-white flex items-center p-4 border-b-2 border-gray-200 text-black">
            {/* Logo Section */}
            <div className="flex items-center gap-4 flex-shrink-0">
                {logoUrl ? (
                    <img src={logoUrl} alt="Logo" className="h-10 w-auto object-contain" />
                ) : (
                    <div className="h-10 w-28 bg-gray-100 flex items-center justify-center text-gray-400 rounded-md">
            ...
            <div className="flex items-center gap-4">
                {displayMetrics.map((metric) => (
                    <div key={metric.id} className="flex items-center gap-2">
                        ...
                        <div className="px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-md text-sm text-gray-800 font-semibold">
                            {metric.value}
                        </div>
                    </div>
                ))}
