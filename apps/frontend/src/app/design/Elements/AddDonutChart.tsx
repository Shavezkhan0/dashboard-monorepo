                    ...
                    {chartProps.isEmpty ? (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                            ...
                            <p className="text-xs text-blue-700 mt-1">
                                Default chart preview shown. Select a dataset above to configure your donut chart.
                            </p>
                        </div>
                    ) : (
...
// Data Mapper for counting and filtering categories from a single column
            ...
            {uniqueCategories.length > 0 && (
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <h4 className="font-semibold text-gray-700">Filter Categories</h4>
                    ...
                    <div className="max-h-32 overflow-y-auto p-2 border bg-white rounded-md space-y-1">
                        {uniqueCategories.map(category => (
                            <div key={category} className="flex items-center">
                                <input id={`cb-donut-${category}`} type="checkbox" checked={selectedCategories.includes(category)} onChange={() => handleCategoryToggle(category)} className="h-4 w-4 rounded border-gray-300" />
                                <label htmlFor={`cb-donut-${category}`} className="ml-2 block text-sm text-gray-900">{String(category)}</label>
                            </div>
                        ))}
                    </div>
                </div>
