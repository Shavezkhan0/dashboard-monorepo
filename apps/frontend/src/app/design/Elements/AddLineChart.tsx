...
const AddLineChart = ({ onClose }) => {
    ...
    const [chartProps, setChartProps] = useState({
        ...
        markerStyle: 'circle',
        markerSize: 8,
        showLegend: false, // Default to false for single dataset
        yStep: 10,
        isEmpty: true
    });
    ...
    return (
        <div className="space-y-4">
                    ...
                    {chartProps.isEmpty ? (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                            ...
                            <p className="text-xs text-blue-700 mt-1">
                                Select a dataset to configure your line chart.
                            </p>
                        </div>
                    ) : (
            ...
            >
                {chartProps.isEmpty ? 'Select Data to Continue' : 'Add Line Chart to Canvas'}
            </button>
        </div>
    );
};

// Enhanced Data Mapper Component with optimized performance
const DataMapper = ({ data, onMap, onBack }) => {
    ...
    const handleGenerate = () => {
        ...
        if (legendField && legendField !== '') {
            ...
            const maxLegendGroups = 10; // Limit legend groups to prevent performance issues
            ...
            // Create datasets for each legend group, but limit combinations
            const datasets = [];
            const maxDatasets = 8; // Limit total datasets for performance
            let datasetCount = 0;
            
            groupedData.forEach((xValueMap, legendValue) => {
                // If we have legend grouping, limit to one Y field per legend to avoid explosion
    ...
    return (
        <div className="space-y-4 p-4 border bg-gray-50 dark:bg-gray-700 rounded-lg">
            <div className="flex items-center justify-between">
                <h4 className="font-semibold text-gray-700 dark:text-gray-200">Configure Line Chart Data</h4>
                <button 
                    onClick={onBack}
            ...
            {legendField && yAxisFields.length > 1 && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                    <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm font-medium text-yellow-800">Performance Note</span>
                    </div>
                    <p className="text-xs text-yellow-700 mt-1">
                        Using legend grouping with multiple Y-axis fields may create many datasets. Consider using one Y-axis field for better performance.
                    </p>
                </div>
            )}
