...
const GaugeChartSetting = ({ initialData, onUpdate, onClose }) => {
    ...
    return (
        <div className="h-full bg-background border-r border-gray-200 flex flex-col shadow-lg pr-2">
            <div className="flex items-center justify-between px-4 py-2 border-b-2 border-gray-200">
                <h2 className="text-lg font-semibold text-foreground">Edit Gauge Chart</h2>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {activeTab === 'data' && (
                    <div className="space-y-4">
                        {/* Gauge Title */}
                        <div>
                            <label className="block text-sm font-medium text-muted-foreground mb-2">Gauge Title</label>
                            <input
                                type="text"
                        ...
                        {selectedDataSet && (
                            <div className="space-y-4">
                                <div className="border border-border rounded-lg p-4 bg-muted">
                                    <h5 className="font-medium text-foreground mb-3">Value Configuration</h5>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">The first value from this column will be displayed</p>
                                    </div>

                                    {/* Data Statistics */}
                                    {valueField && columnStats.values.length > 0 && (
                                        <div className="bg-muted border border-border rounded-lg p-3 mb-4">
                                            <h6 className="text-xs font-medium text-foreground mb-2">Column Statistics</h6>
                                            <div className="grid grid-cols-3 gap-2">
                                                <div className="text-center bg-background rounded p-2">
                                                    <div className="font-bold text-sm text-foreground">{columnStats.min}</div>
                                                ...
                                                <div className="text-center bg-background rounded p-2">
                                                    <div className="font-bold text-sm text-foreground">{columnStats.max}</div>
                                    ...
                                    {valueField && columnStats.values.length > 0 && (
                                        <div>
                                            <label className="block text-sm font-medium text-foreground mb-2">Target Configuration</label>
                                            <div className="grid grid-cols-2 gap-2 mb-2">
                                                <label className={`flex items-center p-2 border-2 rounded cursor-pointer transition-all ${
                                                    targetField === 'manual' ? 'border-indigo-500 bg-indigo-50' : 'border-border bg-background hover:bg-muted'
                                                }`}>
                                                    <input
                                                        type="radio"
                                                        name="targetField"
                                                        ...
                                                        className="h-3 w-3 border-border mr-2 text-indigo-600"
                                                    />
                                                    <span className="text-xs text-foreground font-medium">Manual</span>
                                                </label>
                                                <label className={`flex items-center p-2 border-2 rounded cursor-pointer transition-all ${
                                                    targetField === 'average' ? 'border-indigo-500 bg-indigo-50' : 'border-border bg-background hover:bg-muted'
                                                }`}>
                                                    <input
                                                        type="radio"
                                                        name="targetField"
                                                        ...
                                                        className="h-3 w-3 border-border mr-2 text-indigo-600"
                                                    />
                                                    <span className="text-xs text-foreground font-medium">Avg ({columnStats.avg})</span>
                                                </label>
                                                <label className={`flex items-center p-2 border-2 rounded cursor-pointer transition-all ${
                                                    targetField === 'max' ? 'border-indigo-500 bg-indigo-50' : 'border-border bg-background hover:bg-muted'
                                                }`}>
                                                    <input
                                                        type="radio"
                                                        name="targetField"
                                                        ...
                                                        className="h-3 w-3 border-border mr-2 text-indigo-600"
                                                    />
                                                    <span className="text-xs text-foreground font-medium">Max ({columnStats.max})</span>
                                                </label>
                                                <div>
                                                    <select
                                                        value={targetField !== 'manual' && targetField !== 'average' && targetField !== 'max' ? targetField : ''}
                                                        onChange={(e) => setTargetField(e.target.value)}
                                                        className="w-full p-2 text-xs border border-border rounded text-foreground focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                                    >
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="border border-border rounded-lg p-4">
                            ...
                            {expandedSections.general && (
                                <div className="mt-3 space-y-3">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm text-muted-foreground">Show Target</span>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm text-muted-foreground">Show Range Labels</span>
                        ...
                        <div className="border border-border rounded-lg p-4">
                            <div className="flex items-center justify-between cursor-pointer" onClick={() => toggleSection('value')}>
                                <h3 className="text-sm font-medium text-foreground">Value & Range</h3>
                            ...
                            {expandedSections.value && (
                                <div className="mt-3 space-y-3">
                                    <div>
                                        ...
                                        <input type="number" value={graphData.value} onChange={(e) => handleNumericPropChange('value', e.target.value)} className="w-full p-2 text-sm border border-border rounded text-foreground focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                                    </div>
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <label className="block text-xs font-medium text-muted-foreground mb-1">Min Value</label>
                                            <input type="number" value={graphData.minValue} onChange={(e) => handleNumericPropChange('minValue', e.target.value)} className="w-full p-2 text-sm border border-border rounded text-foreground focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-muted-foreground mb-1">Max Value</label>
                                            <input type="number" value={graphData.maxValue} onChange={(e) => handleNumericPropChange('maxValue', e.target.value)} className="w-full p-2 text-sm border border-border rounded text-foreground focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <label className="block text-xs font-medium text-muted-foreground mb-1">Target</label>
                                            <input type="number" value={graphData.target} onChange={(e) => handleNumericPropChange('target', e.target.value)} className="w-full p-2 text-sm border border-border rounded text-foreground focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-muted-foreground mb-1">Unit / Suffix</label>
                                            <input type="text" value={graphData.unit} onChange={(e) => handlePropChange('unit', e.target.value)} className="w-full p-2 text-sm border border-border rounded text-foreground focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="e.g., K, M, %" />
                                        </div>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="border border-border rounded-lg p-4">
                            ...
                            {expandedSections.appearance && (
                                <div className="mt-3 space-y-3">
                                    ...
                                    <div>
                                        <label className="block text-xs font-medium text-muted-foreground mb-1">Animation Duration (ms)</label>
                                        <input
                                            type="number"
                                            min="0"
                                            max="3000"
                        ...
                        <div className="border border-border rounded-lg p-4">
                            ...
                            {expandedSections.colors && (
                                <div className="mt-3 space-y-3">
                                    <div className="flex items-center justify-between">
                                        <label className="text-sm text-muted-foreground">Needle Color</label>
                                        <input
                                            type="color"
                                    ...
                                    <div className="border-t border-border pt-3">
                                        <h4 className="text-xs font-medium text-foreground mb-2">Gauge Zones</h4>
                                        <div className="space-y-2">
                                            <div className="flex items-center justify-between">
                                                <label className="text-sm text-muted-foreground">Low Zone</label>
                                                <input type="color" value={graphData.colors[0]} onChange={(e) => handleColorChange(0, e.target.value)} className="w-10 h-10 p-1 border border-border rounded cursor-pointer" />
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <label className="text-sm text-muted-foreground">Medium Zone</label>
                                                <input type="color" value={graphData.colors[1]} onChange={(e) => handleColorChange(1, e.target.value)} className="w-10 h-10 p-1 border border-border rounded cursor-pointer" />
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <label className="text-sm text-muted-foreground">High Zone</label>
                                                <input type="color" value={graphData.colors[2]} onChange={(e) => handleColorChange(2, e.target.value)} className="w-10 h-10 p-1 border border-border rounded cursor-pointer" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
